      ...
      <div className="relative neu rounded-[1.25rem] p-5 sm:p-7 h-full">
        {pop && (
          <div className="absolute -top-2.5 left-1/2 -translate-x-1/2">
            <div className="px-4 py-1 rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500 text-[11px] font-bold uppercase tracking-[0.15em] text-white shadow-lg shadow-violet-500/30 whitespace-nowrap">Most Popular</div>
          </div>
        )}

        <div className="flex items-center gap-2 mb-4">
          <div className={`w-2.5 h-2.5 rounded-full bg-gradient-to-r ${t.grad}`} />
          <span className="text-[13px] font-bold uppercase tracking-[0.15em]" style={{color:t.color}}>{pkg.name}</span>
        </div>

        <p className="text-[13px] text-white/40 leading-relaxed mb-4">{pkg.ideal_for}</p>
        ...
        <button onClick={()=>onSelect(pkg.name)} className={`w-full py-3 sm:py-3.5 rounded-xl text-[14px] font-semibold transition-all duration-300 relative overflow-hidden ${pop?'text-white shadow-lg shadow-violet-600/20':'text-white/60 hover:text-white'}`}
