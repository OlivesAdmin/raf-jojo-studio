...
export function ThankYouPage() {
  ...
  return (
    <div className="max-w-[600px] mx-auto px-4 sm:px-8 py-16 sm:py-20 text-center anim-up overflow-x-hidden">
      ...
      <p className="text-[15px] sm:text-[15px] text-white/50 mb-8 sm:mb-10 max-w-md mx-auto leading-relaxed">Our team has your brief and will start immediately.</p>
      <div className="neu rounded-2xl p-5 sm:p-7 text-left mb-8 sm:mb-10">
        <h3 className="font-bold text-white/85 mb-4 sm:mb-5 text-[15px] sm:text-[15px]" style={{ fontFamily: "'Cabinet Grotesk', sans-serif" }}>What happens next?</h3>
        {[{ i: "📧", t: "Confirmation email sent", d: "Check inbox for receipt" }, { i: "👋", t: "Team assigned (2 hours)", d: "Dedicated lead for your project" }, { i: "📞", t: "Kickoff message (24 hours)", d: "We'll clarify any questions" }, { i: "⚡", t: "Work begins", d: "We build & keep you updated" }].map((s, i) => (
          <div key={i} className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-4 last:mb-0">
            ...
            <div className="min-w-0"><h4 className="text-[14px] sm:text-[14px] font-semibold text-white/65">{s.t}</h4><p className="text-[14px] sm:text-[15px] text-white/40">{s.d}</p></div>
          </div>
        ))}
      </div>
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2.5 sm:gap-3">
        <button onClick={() => navigate("services")} className="px-6 sm:px-7 py-2.5 sm:py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white text-[14px] sm:text-[15px] font-semibold rounded-full">More Services</button>
        <button onClick={() => navigate("home")} className="px-6 sm:px-7 py-2.5 sm:py-3 border border-white/[0.06] text-[14px] sm:text-[15px] text-white/55 rounded-full hover:border-white/[0.12] transition-colors">Home</button>
      </div>
      <p className="text-[9px] sm:text-[14px] text-white/10 mt-8 sm:mt-10">Order #RJ-{Math.random().toString(36).substring(2,8).toUpperCase()}</p>
    </div>
  );
}
