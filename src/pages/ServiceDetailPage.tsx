  ...
  return (
    <div className="overflow-x-hidden pb-20 sm:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-4 sm:pt-6">
        <div className="flex items-center gap-2 text-[10px] sm:text-[10.5px] text-white/20 overflow-x-auto whitespace-nowrap">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-10 sm:py-16 relative">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div className="anim">
            ...
            <p className="text-[13px] sm:text-[14.5px] text-white/30 leading-relaxed mb-5 max-w-lg">{service.description}</p>
            <div className="flex flex-wrap gap-2 text-[10.5px] sm:text-[11.5px]">
              {[`From $${service.packages[0].price}`, `${Math.min(...service.packages.map(p=>p.delivery_days))}-day delivery`, `Up to ${Math.max(...service.packages.map(p=>p.revisions))} revisions`].map(t => (
                <span key={t} className="text-white/25 bg-white/[0.03] px-2.5 py-1 rounded-full border border-white/[0.04]">{t}</span>
              ))}
            </div>
          </div>
          {/* Service hero image */}
      ...
      <section className="py-12 sm:py-16 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          <div className="text-center mb-8 sm:mb-10">
            ...
            <p className="text-[11.5px] sm:text-[12.5px] text-white/20 mt-2">Full ownership & source files included</p>
          </div>
        ...
        <div className="neu rounded-2xl overflow-hidden"><div className="overflow-x-auto">
          <table className="w-full min-w-[480px]">
            <thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
              <th className="text-left py-3 sm:py-4 px-4 sm:px-5 text-[10.5px] sm:text-[11px] font-medium text-white/25 w-[40%]">Feature</th>
              {service.packages.map(p=><th key={p.name} className={`text-center py-3 sm:py-4 px-3 sm:px-5 text-[10.5px] sm:text-[11px] font-bold ${p.popular?'text-violet-400':'text-white/40'}`}>{p.name}</th>)}
            </tr></thead>
            <tbody>
              <tr style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}><td className="py-2.5 px-4 sm:px-5 text-[10.5px] text-white/25">Price</td>{service.packages.map(p=><td key={p.name} className="text-center py-2.5 px-3 sm:px-5 text-[10.5px] font-bold text-white/70">${p.price.toLocaleString()}</td>)}</tr>
              <tr style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}><td className="py-2.5 px-4 sm:px-5 text-[10.5px] text-white/25">Delivery</td>{service.packages.map(p=><td key={p.name} className="text-center py-2.5 px-3 text-[10.5px] text-white/30">{p.delivery_days}d</td>)}</tr>
              <tr style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}><td className="py-2.5 px-4 sm:px-5 text-[10.5px] text-white/25">Revisions</td>{service.packages.map(p=><td key={p.name} className="text-center py-2.5 px-3 text-[10.5px] text-white/30">{p.revisions}</td>)}</tr>
              {[...new Set(service.packages.flatMap(p=>p.features))].map((f,i)=>(
                <tr key={i} style={{borderBottom:'1px solid rgba(255,255,255,0.02)'}}>
                  <td className="py-2 px-4 sm:px-5 text-[9.5px] sm:text-[10.5px] text-white/20">{f}</td>
      ...
      {service.addons.length > 0 && <section className="py-12 sm:py-16"><div className="max-w-[1000px] mx-auto px-4 sm:px-8">
        ...
        <p className="text-[11.5px] text-white/20 mb-6 sm:mb-8">Enhance any package</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">{service.addons.map(a=>(
          <button key={a.id} onClick={()=>tog(a.id)} className={`text-left p-4 sm:p-5 rounded-xl border transition-all duration-300 ${addons.includes(a.id)?'border-violet-500/30 bg-violet-500/5 shadow-lg shadow-violet-500/5':'border-white/[0.04] bg-white/[0.01] hover:border-white/[0.08]'}`}>
            <div className="flex items-center justify-between mb-1"><h4 className="font-semibold text-[11.5px] sm:text-[12.5px] text-white/70">{a.title}</h4><span className="font-bold text-[11.5px] text-violet-400/80">+${a.price}</span></div>
            <p className="text-[10px] text-white/20">{a.description}</p>
            {addons.includes(a.id)&&<span className="inline-flex items-center gap-1 mt-1.5 text-[10px] text-violet-400 font-medium">✓ Selected</span>}
          </button>
        ))}</div>
      </div></section>}
      ...
      <section className="py-12 sm:py-16"><div className="max-w-[1000px] mx-auto px-4 sm:px-8">
        ...
        {service.delivery_process.map((s,i)=>(
          <div key={i} className="flex items-start gap-4 pb-6 relative">
            ...
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[9px] sm:text-[10px] font-bold text-white flex-shrink-0 relative z-10 shadow-lg" style={{background:`linear-gradient(135deg, ${cc}, ${cc}99)`,boxShadow:`0 4px 12px ${cc}25`}}>{i+1}</div>
            <p className="text-[12px] text-white/35 pt-1">{s}</p>
          </div>
        ))}
      </div></section>
        ...
        {service.faq.map((f,i)=>(
          <div key={i} className="neu rounded-xl overflow-hidden mb-2">
            <button onClick={()=>setFaq(faq===i?null:i)} className="w-full flex items-center justify-between px-4 sm:px-5 py-3.5 text-left">
              <span className="text-[11.5px] sm:text-[12.5px] font-medium text-white/70 pr-4">{f.question}</span>
              <svg className={`w-4 h-4 text-white/20 transition-transform flex-shrink-0 ${faq===i?'rotate-180':''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M6 9l6 6 6-6"/></svg>
            </button>
            {faq===i&&<div className="px-4 sm:px-5 pb-3.5"><p className="text-[10.5px] text-white/25 leading-relaxed">{f.answer}</p></div>}
          </div>
        ))}
      </div></section>
      ...
      {cross.length>0&&<section className="py-12 sm:py-16"><div className="max-w-7xl mx-auto px-4 sm:px-8">
        ...
        <p className="text-[11px] text-white/20 mb-5 sm:mb-8">Combine for better results</p>
          ...
          return (
            <button key={cs.slug} onClick={()=>navigate("service-detail",{slug:cs.slug})} className="group text-left neu rounded-xl overflow-hidden flex">
              ...
              <div className="p-4 sm:p-5 flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1"><span className="text-[16px]">{cs.icon}</span><h3 className="font-bold text-white/70 group-hover:text-violet-300 transition-colors text-[13px] truncate">{cs.name}</h3></div>
                <p className="text-[10.5px] text-white/20 mb-1.5 line-clamp-2">{cs.outcome_statement}</p>
                <span className="text-[10px] font-semibold text-violet-400/60">From ${cs.packages[0].price} →</span>
              </div>
            </button>
          );
        })}</div>
      ...
      <div className="fixed bottom-0 inset-x-0 py-2.5 px-4 z-40 sm:hidden" style={{background:'rgba(5,5,16,0.95)',backdropFilter:'blur(20px)',borderTop:'1px solid rgba(255,255,255,0.04)'}}>
        <div className="flex items-center justify-between">
          <div className="min-w-0 mr-3"><p className="text-[12px] font-bold text-white/80 truncate">{service.name}</p><p className="text-[10px] text-white/20">From ${service.packages[0].price}</p></div>
          <button onClick={()=>window.scrollTo({top:500,behavior:'smooth'})} className="px-5 py-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white text-[11px] font-semibold rounded-full flex-shrink-0">Buy Now</button>
        </div>
      </div>
    </div>
  );
